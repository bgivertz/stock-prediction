...
def pull_csvs_from_yahoo_finance(path):
    for stock_index in config.stock_listing_symbols:
        data_as_dataframe = yf.download(stock_index, start=config.date_range_start, end=config.date_range_end,
                                        progress=False)
        data_as_dataframe.to_csv(path + '/' + stock_index + '.csv')
